...
///
/// In normal mode: every second, it checks the default session's WeChat process
/// by running a11y → identify. If WeChat has crashed, it restarts it. If no IA
/// state has been identified for more than 60 seconds, it kills and restarts it.
/// In RECEIVE_ONLY mode: use only process/DB signals. We do not rely on a11y
/// for steady-state health signals (defaults to 1-second loop still).
pub fn spawn_health_monitor() {
    tokio::spawn(async move {
        ...
        let mut last_ro_a11y_probe = Instant::now();
        ...
        loop {
            ...
            if MONITORING_PAUSED.load(Ordering::Relaxed) {
                ...
                last_ro_a11y_probe = Instant::now();
                continue;
            }
            ...
            if config::receive_only() {
                let account_dir = find_account_dir(wechat_pid);
                if account_dir.is_some() {
                    last_identified = Instant::now();
                } else {
                    tracing::info!(
                        "[health] WeChat running but account dir missing (logged_out candidate), pid={}",
                        wechat_pid
                    );
                }

                let ro_probe_secs = config::receive_only_a11y_probe_secs();
                if ro_probe_secs > 0 && last_ro_a11y_probe.elapsed().as_secs() >= ro_probe_secs {
                    let exec_options = ExecOptions {
                        session: Some(session.clone()),
                        timeout_ms: 10_000,
                    };
                    if let Err(err) = get_a11y_desktop(&exec_options).await {
                        tracing::warn!(
                            "[health] Optional RECEIVE_ONLY a11y probe failed for pid={}: {err}",
                            wechat_pid
                        );
                    } else {
                        tracing::debug!(
                            "[health] Optional RECEIVE_ONLY a11y probe succeeded for pid={}",
                            wechat_pid
                        );
                    }
                    last_ro_a11y_probe = Instant::now();
