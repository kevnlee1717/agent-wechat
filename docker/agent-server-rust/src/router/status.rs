...
pub async fn logout() -> Json<serde_json::Value> {
    ...
    // context 需在外层声明：logout FSM（下方 run_execution_loop）要 &mut context。
    // RECEIVE_ONLY 下 create_context 只读 DB（不碰 a11y），无害。
    let mut context = {
        let db = get_db();
        create_context(session.clone(), &db)
    };

    let is_logged_in = if config::receive_only() {
        let wechat_pid = find_wechat_pid();
        receive_only_auth_status(wechat_pid, wechat_pid.and_then(find_account_dir)) == "logged_in"
    } else {
...
fn receive_only_auth_status(
    wechat_pid: Option<i64>,
    account_dir: Option<String>,
) -> &'static str {
    match wechat_pid {
        None => "app_not_running",
        Some(_) if account_dir.is_some() => "logged_in",
        Some(_) => "logged_out",
    }
}
...
#[cfg(test)]
mod tests {
    use super::receive_only_auth_status;

    #[test]
    fn receive_only_auth_status_app_not_running_when_no_pid() {
        assert_eq!(
            receive_only_auth_status(None, None),
            "app_not_running"
        );
    }

    #[test]
    fn receive_only_auth_status_logged_in_when_account_dir_exists() {
        assert_eq!(
            receive_only_auth_status(Some(12345), Some("xwechat_files/abc".to_string())),
            "logged_in"
        );
    }

    #[test]
    fn receive_only_auth_status_logged_out_when_no_account_dir() {
        assert_eq!(
            receive_only_auth_status(Some(12345), None),
            "logged_out"
        );
    }
}
